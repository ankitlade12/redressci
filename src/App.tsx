...
import { EvidenceDiscoveryPanel, FailureRadarPage, GitHubCheckPanel, LiveVerificationPanel, ReporterLinkPanel, ReporterStatusPage, VoiceInputButton } from "./product-features";
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Main navigation">
        ...
        {role !== "reporter" && <button className={page === "radar" ? "active" : ""} onClick={() => onNavigate("radar")}><Icon name="search" />Failure radar</button>}
...
function CaseDetail({ item, onBack, refresh, role }: { item: RedressCase; onBack: () => void; refresh: () => Promise<void>; role: WorkspaceRole }) {
  ...
  return <div className="page case-page">
    ...
    <div className="case-content">
      ...
      {tab === "overview" && <Overview item={item} setTab={setTab} role={role} />}
      {tab === "evidence" && <EvidenceView item={item} />}
      {tab === "evaluation" && <EvaluationView item={item} setTab={setTab} />}
      {tab === "validation" && <ValidationView item={item} runs={freshRuns} onRun={validate} running={running} role={role} refresh={refresh} />}
...
function ReviewWorkspace({ item, refresh, setTab, role }: { item: RedressCase; refresh: () => Promise<void>; setTab: (tab: CaseTab) => void; role: WorkspaceRole }) {
  ...
  if (!item.evaluation) {
    ...
    return <section className="review-workspace card">
      ...
      {error && <div className="form-error"><Icon name="alert" size={15} />{error}</div>}
      <EvidenceDiscoveryPanel item={item} onUse={(suggestion) => setReview((current) => ({ ...current, sourceTitle: suggestion.title, sourceLocator: suggestion.locator, sourceExcerpt: suggestion.excerpt }))} />
...
export function Overview({ item, setTab, role = "admin" }: { item: RedressCase; setTab: (tab: CaseTab) => void; role?: WorkspaceRole }) {
  ...
  return <div className="two-column">
    <div className="content-stack">
      <section className="card story-card"><div className="card-label"><Icon name="user" size={16} /> THE REPORTED EXPERIENCE</div><blockquote>“{item.userInput}”</blockquote><div className="observed"><span>AI RESPONSE</span><p>{item.observedResponse}</p></div><div className="impact"><strong>Why this matters</strong><p>{item.description}</p></div></section>
    ...
    <aside className="content-stack">
      ...
      <ReporterLinkPanel item={item} role={role} />
    </aside>
  </div>;
}
...
export function ValidationView({ item, runs, onRun, running, role = "admin", refresh = async () => undefined }: { item: RedressCase; runs: { broken: EvaluationRun; fixed: EvaluationRun } | null; onRun: () => void; running: boolean; role?: WorkspaceRole; refresh?: () => Promise<void> }) {
  ...
  return <div><div className="validation-title"><div><span className="eyebrow"><span /> RECORDED-RESPONSE PROOF</span><h2>The test must catch the failure<br />and recognize the correction.</h2><p>This gate compares reviewer-approved recorded responses; it does not call a deployed system.</p></div><button className="button dark" onClick={onRun} disabled={running || !item.evaluation}><Icon name="refresh" size={17} className={running ? "spin" : ""} />{running ? "Running both targets…" : item.evaluation ? available ? "Re-run validation" : "Run validation gate" : "Generate evaluation first"}</button></div>
    ...
    <LiveVerificationPanel item={item} role={role} refresh={refresh} />
  </div>;
}
...
export default function App() {
  ...
  useEffect(() => { if (route.page === "status") setLoading(false); else load().catch(() => setLoading(false)); }, []);
  useEffect(() => {
